...
export interface SupabaseUserData {
  ...
  /** True when profile.is_internal === true and profile.role === "admin" (can delete clients). */
...
export async function fetchSupabaseUserData(userId: string): Promise<SupabaseUserData | null> {
  ...
  try {
    ...
    const isNobaInternalAdminUser = isNobaInternalAdmin(p)
    ...
    return {
      ...
      entity,
      isNobaProducerUser,
      isNobaInternalAdmin: isNobaInternalAdminUser,
    }
  } catch (error) {
