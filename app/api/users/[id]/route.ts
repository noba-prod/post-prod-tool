...
export async function GET(
  ...
  const canViewSameOrg =
    ...
    profile.organization_id === targetProfile.organization_id

  if (!isInternal && !canViewSameOrg) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 })
  }

  return NextResponse.json({ user: mapProfileToUser(targetProfile) })
}
...
export async function PATCH(
  ...
  return NextResponse.json({ user: mapProfileToUser(updatedProfile) })
}
