...
import { isNobaInternalAdmin } from "@/lib/auth/noba-internal-admin"
import { createClient } from "@/lib/supabase/server"
import { createAdminClient } from "@/lib/supabase/admin"
import { detachPlayerFromCollections } from "@/lib/services/players/detach-player-from-collections"
...
export async function DELETE(
  ...
  const adminClient = createAdminClient()
  const playersTable = adminClient.from("players") as ReturnType<typeof adminClient.from>
  const { data: playerRow, error: playerLookupError } = await playersTable
    .select("id, type")
    .eq("id", playerId)
    .maybeSingle()

  if (playerLookupError) {
    return NextResponse.json({ error: playerLookupError.message }, { status: 500 })
  }
  if (!playerRow) {
    return NextResponse.json({ error: "Player not found" }, { status: 404 })
  }

  const playerType = (playerRow as { type: string }).type
  if (playerType === "client" && !isNobaInternalAdmin(profile)) {
    return NextResponse.json(
      {
        error: "Only a noba administrator can delete clients.",
        code: "CLIENT_DELETE_REQUIRES_ADMIN",
      },
      { status: 403 }
    )
  }
  ...
  if (playerType !== "client") {
    try {
      await detachPlayerFromCollections(adminClient, playerId)
    } catch (detachError) {
      const message =
        detachError instanceof Error ? detachError.message : "Failed to unlink player from collections"
      return NextResponse.json({ error: message }, { status: 500 })
    }
  }

  const { error: deleteError } = await playersTable.delete().eq("id", playerId)
